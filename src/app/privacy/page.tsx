...
export default async function PrivacyPage() {
  ...
  return <main className="legal-page"><section className="legal-card"><div className="legal-topline"><Link href="/" className="back-link">← {tr(locale,"На главную","Home")}</Link><LanguageSwitcher locale={locale} /></div><span className="eyebrow">{appName}</span><h1>{tr(locale,"Конфиденциальность","Privacy")}</h1><p className="legal-lead">{tr(locale,`Кратко о том, какие данные использует ${appName}.`,`A concise overview of the data ${appName} uses.`)}</p>
