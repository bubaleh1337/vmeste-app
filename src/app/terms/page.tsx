...
export default async function TermsPage() {
  ...
  return <main className="legal-page"><section className="legal-card"><div className="legal-topline"><Link href="/" className="back-link">← {tr(locale,"На главную","Home")}</Link><LanguageSwitcher locale={locale} /></div><span className="eyebrow">{appName}</span><h1>{tr(locale,"Условия использования","Terms of use")}</h1><p className="legal-lead">{tr(locale,`Текущая версия ${appName} предназначена для личного и совместного отслеживания накоплений.`,`The current ${appName} version is intended for personal and shared savings tracking.`)}</p>
