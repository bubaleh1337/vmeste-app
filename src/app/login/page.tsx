...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string }> }) {
  ...
  if (!isSupabaseConfigured()) return <main className="auth-page"><section className="auth-card"><div className="auth-card-top"><span className="brand-mark large">{APP_NAME.slice(0,1).toUpperCase()}</span><LanguageSwitcher locale={locale} /></div><span className="eyebrow">{tr(locale,"Локальная разработка","Local development")}</span><h1>{tr(locale,"Хранилище данных ещё не подключено","Data storage is not connected")}</h1><p>{tr(locale,"Локальный демонстрационный режим остаётся доступным на главной странице.","The local demo remains available on the home page.")}</p><Link className="secondary-button auth-link" href="/">{tr(locale,"Вернуться на главную","Back home")}</Link></section></main>;
