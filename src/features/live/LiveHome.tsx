...
import { formatMoney, visualProgressPercent } from "@/lib/money";
import type { LiveGoalCard, LiveProfile } from "./types";

export function LiveHome({ profile, goals, error }: { profile: LiveProfile; goals: LiveGoalCard[]; error?: string }) {
  ...
  return (
    <div className="live-shell">
      ...
      <main className="live-main"><section className="page-section compact-page">
        ...
        <div className="live-goal-grid">{goals.map((goal) => <article className="goal-card live-goal-card" key={goal.id}><Link className="live-goal-main-link" href={`/goals/${goal.id}`}><div className="goal-card-top"><div><span className="eyebrow">{goal.role === "owner" ? tr(locale,"Владелец","Owner") : tr(locale,"Участник","Member")}</span><h2>{goal.title}</h2></div><span className="goal-arrow">↗</span></div><div className="goal-target-copy"><span>{tr(locale,"Сумма цели","Goal amount")}</span><strong>{formatMoney(goal.targetAmountMinor, goal.currencyCode, localeTag(locale))}{goal.progressIncomplete ? "*" : ""}</strong></div><div className="goal-progress-copy"><span>{tr(locale,"Накоплено","Saved")}</span><strong>{goal.progressPercent.toFixed(1)}%</strong></div><div className="progress-track live-goal-progress" role="progressbar" aria-label={tr(locale,"Прогресс накоплений","Savings progress")} aria-valuemin={0} aria-valuemax={100} aria-valuenow={visualProgressPercent(goal.progressPercent)}><span style={{width:`${visualProgressPercent(goal.progressPercent)}%`}} /></div><div className="goal-saved-row"><span>{formatMoney(goal.actualSavedMinor, goal.currencyCode, localeTag(locale))}</span></div><div className="goal-card-bottom"><span>{tr(locale,"до","by")} {formatDate(goal.targetDate)}</span><span>{goal.status === "archived" ? tr(locale,"Архив","Archived") : tr(locale,"Активна","Active")}</span></div></Link>{goal.status !== "archived"&&<Link className="primary-button live-goal-add-button" href={`/goals/${goal.id}?panel=savings&add=saving#add-saving`}>＋ {tr(locale,"Добавить накопление","Add savings")}</Link>}{goal.progressIncomplete&&<small className="live-goal-fx-note">* {tr(locale,"Часть валют временно не включена: курс недоступен.","Some currencies are temporarily excluded because rates are unavailable.")}</small>}</article>)}</div>
