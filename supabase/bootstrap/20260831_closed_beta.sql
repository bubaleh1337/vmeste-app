...
-- ============================================================================
-- BEGIN: 202609010003_pdf_import.sql
-- ============================================================================
-- Stage 4.5: PDF statement import + currency-aware imported savings.
-- Safe for existing data. Original files are never stored in the database.

alter type public.transaction_source add value if not exists 'pdf';

alter table public.imports drop constraint if exists imports_file_type_check;
alter table public.imports add constraint imports_file_type_check check (file_type in ('csv', 'xlsx', 'pdf'));
...
begin
  ...
  loop
    ...
    v_fingerprint := private.import_fingerprint(
      p_goal_id, v_date, v_amount, v_description, v_participant, v_type || ':' || v_currency
    );
    v_is_duplicate := v_fingerprint = any(v_seen);
...
begin
  ...
  loop
    ...
    v_fingerprint := private.import_fingerprint(
      p_goal_id, v_date, v_amount, v_description, v_participant, v_type || ':' || v_row_currency
    );
    v_duplicate := exists (
      select 1 from public.import_rows ir
      join public.imports i on i.id = ir.import_id
    ...
    if not v_duplicate and p_target_kind = 'savings' then
      v_duplicate := exists (
        select 1 from public.savings_transactions s
        where s.goal_id = p_goal_id and s.deleted_at is null and s.transaction_date = v_date
          and s.amount_minor = v_amount and s.currency_code = v_row_currency
          and s.contributor_user_id = v_participant and s.type::text = v_type
          and private.normalize_import_description(s.description) = private.normalize_import_description(v_description)
      );
    elsif not v_duplicate and p_target_kind = 'expenses' then
      v_duplicate := exists (
...
-- ============================================================================
-- END: 202609010003_pdf_import.sql
-- ============================================================================

-- ============================================================================
-- BEGIN: 202609010004_duplicate_protection.sql
-- ============================================================================
...
-- ============================================================================
-- END: 202609010004_duplicate_protection.sql
-- ============================================================================
