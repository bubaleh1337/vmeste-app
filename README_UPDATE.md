...
- pull-to-refresh на странице цели с автоматическим сворачиванием всех раскрытых блоков;
- «Подробнее» раскрывается сразу под той карточкой, по которой нажали;
- переход в импорт из накоплений/расходов заранее выбирает правильный тип импорта;
- после успешного импорта кнопка явно меняется на «Импортировано ✓», а подтверждение успеха показывается рядом с ней.

SQL и `npm install` не нужны.
