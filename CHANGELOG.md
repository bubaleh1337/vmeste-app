...
- На странице цели добавлен заметный отступ между плашкой роли и финансовыми карточками.
- Для стрелки раскрытия выделена отдельная нижняя зона карточки, а подпись «Подробнее» / `Details` отодвинута от неё.
- Исправление адаптировано для мобильных и компьютерных экранов; SQL-миграции и новые npm-пакеты не требуются.

## 0.18.0 — 2026-09-08

- Каждый участник получил собственный цвет вклада, который может изменить только он сам в блоке «Участники» / `Members`.
- Общая полоса накоплений на главной странице и в обзоре цели разделена на цветные сегменты участников.
- Рядом с полосой остаётся текстовая легенда с именами и суммами, поэтому цвет не является единственным способом прочитать данные.
- Для существующих участников автоматически используются стабильные спокойные цвета до первого ручного выбора.
- Изменения адаптированы для русского и английского интерфейсов, телефона и компьютера.
- Требуется миграция `supabase/migrations/202609080001_participant_colors.sql`; новые npm-пакеты не требуются.

## 0.17.1 — 2026-09-04

- На главной странице целей общая сумма цели снова показана главным числом.
- Под суммой цели сохранены компактные процент и полоса прогресса, а ниже — фактически накопленная сумма.
- «Добавить расход» оформлено такой же заметной основной кнопкой, как «Добавить накопление».
- Изменения локализованы для русского и английского интерфейсов и адаптированы для мобильных и компьютерных экранов.
- SQL-миграции и новые npm-пакеты не требуются.

## 0.17.0 — 2026-09-04

- Приложение переименовано в «Копим вместе» / Saving Together во всех русских и английских интерфейсах, метаданных и PWA-манифесте.
- На главной странице каждая цель показывает накопленную сумму, процент и полосу прогресса.
- На карточку цели добавлена заметная кнопка быстрого ручного пополнения, сразу открывающая нужную форму.
- Ссылка ручного пополнения внутри цели оформлена как полноценная основная кнопка.
- Подтверждение отрицательной корректировки скрыто для обычных операций и появляется только если корректировка действительно уведёт валютный баланс ниже нуля.
- Главная страница обновляется после ручного пополнения; SQL-миграции и новые npm-пакеты не требуются.

## 0.15.1 — 2026-09-02

- Добавлено восстановление сохранённой авторизации после запуска установленного iPhone PWA, возврата приложения из фона и восстановления сети.
- Добавлены недостающие Safari-совместимые реализации `Promise.withResolvers` и асинхронного итератора `ReadableStream` перед запуском PDF.js.
- Halyk PDF с расходами продолжает обрабатываться локально в браузере и распознаёт только дебетовые операции.
- Добавлены регрессионные тесты восстановления сессии и браузерной совместимости PDF.
- Миграции Supabase и новые npm-пакеты не требуются.

## 0.15.0 — 2026-09-01

- Добавлена добровольная поддержка проекта через Buy Me a Coffee на публичной главной странице и в разделе помощи.
- Платёжная страница открывается в новой вкладке; приложение не принимает и не хранит платёжные данные.
- Добавлены русские и английские тексты, мобильная раскладка и регрессионные тесты внешней ссылки.
- Миграции Supabase и новые npm-пакеты не требуются.
